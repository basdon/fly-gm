...
hook OnGameModeInit()
{
	...
	cache_delete apc
	apc = mysql_query(1, "SELECT a,s,h,x,y,z,v FROM rnw")
	rowcount = cache_get_row_count()
	while (rowcount--) {
		new aptindex, specifier[2], Float:heading, Float:x, Float:y, Float:z, vor
		...
		cache_get_field_int(rowcount, 6, vor)
		APT_AddRunway aptindex, specifier[0], heading, x, y, z, vor
	}
	cache_delete apc
}
